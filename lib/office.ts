...
function databaseForExport(result: ResearchResult) {
  const raw: any = result.database || {};
  const list = (name: string) => result.lists.find(item => item.name === name)?.items || [];
  const mapNodes = (index: number) => result.maps[index]?.nodes || [];
  const use = (value: any, fallback: any[]) => Array.isArray(value) && value.length >= fallback.length ? value : fallback;
  const clusters = list("产业集群清单");
  const policies = list("政策清单");
  const enterprises = list("重点企业清单");
  const projects = list("重点项目清单");
  const problems = list("产业问题清单");
  return {
    ...raw,
    industryChain: use(raw.industryChain, mapNodes(0).map((item, index) => ({ id: `CH${String(index + 1).padStart(3, "0")}`, layer: "产业链", segment: item.label, output: item.detail, representatives: "见重点企业清单", maturity: "待专项核验", barrier: "待专项核验", bottleneck: item.detail, impact: "影响产品性能、成本或交付", entryMode: "联合验证/产业协同", marketProxy: "待补充可比市场或盈利指标", sourceIds: item.evidenceIds || [] }))),
    products: use(raw.products, mapNodes(1).map((item, index) => ({ id: `PR${String(index + 1).padStart(3, "0")}`, track: item.label, product: item.label, company: "见重点企业清单", user: "老人/照护人员/机构", task: item.detail, scenario: item.label, buyer: "医院、养老机构或家庭", payer: "采购方/服务购买方", businessModel: "销售、租赁或服务包待核", price: "未公开/待询价", qualification: "逐产品核验", commercialization: "见场景与企业证据", risk: "利用率、合规、安全与支付", action: "进入场景验证", sourceIds: item.evidenceIds || [] }))),
    technologies: use(raw.technologies, mapNodes(2).map((item, index) => ({ id: `TE${String(index + 1).padStart(3, "0")}`, layer: "技术路线", direction: item.label, description: item.detail, trl: "研究判断，待核", penetration: "待核", bottleneck: item.detail, gap: "待专项比较", mainstreamTime: item.label, representatives: "见重点企业清单", verificationMetric: "任务成功率、人工接管率、安全与利用率", action: "按里程碑验证", sourceIds: item.evidenceIds || [] }))),
    regions: use(raw.regions, clusters.map((item, index) => ({ id: `RG${String(index + 1).padStart(3, "0")}`, tier: item.type, region: item.region, cluster: item.name, enterprises: "见重点企业清单", research: "待核", platforms: "待核", capital: "待核", manufacturing: "待核", scenarios: item.summary, strength: item.summary, weakness: "具体载体、权限与规模待核", role: item.status || item.nextAction, sourceIds: item.evidenceIds || [] }))),
    policies: use(raw.policies, policies.map((item, index) => ({ id: `PO${String(index + 1).padStart(3, "0")}`, level: item.type, name: item.name, date: "见来源索引", publisher: (item.evidenceIds || []).map(id => result.sources.find(source => source.id === id)?.publisher).filter(Boolean).join("；"), supportObject: result.meta.industry, supportMethod: item.summary, constraints: "申报条件、有效期和预算来源待逐项核验", impact: item.summary, opportunity: item.nextAction, status: item.status, sourceIds: item.evidenceIds || [] }))),
    enterprises: use(raw.enterprises, enterprises.map((item, index) => ({ id: `EN${String(index + 1).padStart(3, "0")}`, name: item.name, type: item.type, region: item.region, segment: item.type, products: item.summary, technology: "见产品与技术路线表", qualification: "逐产品核验", commercialEvidence: item.status, financialEvidence: "非上市主体需财务尽调", financing: "待核", shareholders: "待核", collaboration: item.nextAction, risk: item.summary, dueDiligence: item.nextAction, sourceIds: item.evidenceIds || [] }))),
    projects: use(raw.projects, projects.map((item, index) => ({ id: `PJ${String(index + 1).padStart(3, "0")}`, name: item.name, type: item.type, date: result.meta.cutoff, amount: "待可研/尽调", participants: "实施主体与合作方待定", location: item.region, status: item.status, useOfFunds: "设备、平台、验证或运营投入", value: item.summary, request: item.nextAction, risk: item.summary, nextAction: item.nextAction, sourceIds: item.evidenceIds || [] }))),
    problems: use(raw.problems, problems.map((item, index) => ({ id: `IS${String(index + 1).padStart(3, "0")}`, type: item.type, segment: item.type, description: item.name, cause: item.summary, impact: item.summary, severity: item.status, solution: item.nextAction, owner: "实施主体/机构/企业", timing: "近期", metric: "按问题建立量化验证指标", restartCondition: item.nextAction, sourceIds: item.evidenceIds || [] }))),
    resources: use(raw.resources, result.dataGaps.filter(gap => gap.module === "主体资源").map((gap, index) => ({ id: `RS${String(index + 1).padStart(3, "0")}`, ownership: "待内部核实", name: gap.item, type: "主体资源", location: result.meta.region, evidence: gap.impact, capacity: "待核", collaboration: gap.nextAction || gap.method || "内部核实", controlBoundary: "未确认前不得视为主体自有", verification: gap.nextAction || gap.method || "内部核实", sourceIds: [] }))),
    scoreItems: use(raw.scoreItems, result.scores.map(item => ({ category: item.type, name: item.name, stage: item.status, dimensions: [], total: item.score, coverage: item.evidenceIds.length ? "有公开证据，仍需尽调" : "待补证", status: item.status, rationale: item.rationale, nextAction: "进入对应尽调、试点或谈判流程", sourceIds: item.evidenceIds || [] }))),
    scoreWeights: Array.isArray(raw.scoreWeights) ? raw.scoreWeights : [],
    marketMetrics: use(raw.marketMetrics, [{ id: "MM-PENDING", metric: `${result.meta.industry}目标细分市场统一口径`, metricType: "数据缺口", caliber: "不得将广义机器人、银发经济或康复设备规模直接替代养老机器人市场", region: result.meta.region, year: result.meta.cutoff.slice(0, 4), value: "待专项核验", unit: "—", growth: "不适用", dataNature: "待核事项", comparability: "缺少可比的一手统计口径，正式研究应保留各来源口径并解释差异", sourceIds: [] }]),
    roiScenarios: use(raw.roiScenarios, [{ id: "ROI-PENDING", scenario: "养老机器人场景经济性", case: "暂不测算", initialInvestment: "待报价", annualRevenue: "待支付机制", annualSavings: "待试点工时基线", annualOpex: "待运维报价", annualNetBenefit: "待核", paybackYears: "待核", roiOrNpv: "证据不足，不输出收益结论", assumptions: "需取得设备报价、机构人员工时、利用率、维护成本和支付合同", sensitivity: "利用率、人工接管率、维护成本、续费率", boundary: "数据补齐前仅作为尽调任务，不构成投资建议", sourceIds: [] }]),
    changeLog: use(raw.changeLog, [{ date: result.meta.cutoff, module: "示范数据库", changeType: "自动映射", description: "由四图、五清单、评分和来源映射为专业底表", before: "网页摘要", after: "可下载结构化底表", status: "完成" }]),
    searchLog: use(raw.searchLog, [
      { queryId: "Q01", round: "第一轮", topic: "产业全景", query: "示范成果来源汇总", preferredSources: "政府、监管、企业官网与公告", result: `形成${result.sources.length}条来源记录`, newRecords: String(result.sources.length), duplication: "已去重", evidenceQuality: "见来源等级", saturation: "进入重点对象核验" },
      { queryId: "Q02", round: "第二轮", topic: "重点对象与缺口", query: "政策、企业、项目、区域与合规补充核验", preferredSources: "一手政府文件、公告与企业原始页面", result: `形成${result.dataGaps.length}项待核任务`, newRecords: "见各底表", duplication: "已去重", evidenceQuality: "见来源等级", saturation: "示范数据；正式任务按当前截止日重新检索" },
    ]),
  };
}

function buildDetailedSlides(result: ResearchResult) {
  const db: any = databaseForExport(result);
  const slides: string[] = [];
...
export function buildXlsxBytes(result: ResearchResult) {
  const db = databaseForExport(result);
  const sourceById = new Map(result.sources.map(source => [source.id, source]));
  ...
  add("xl/styles.xml", `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><fonts count="3"><font><sz val="10"/><name val="Microsoft YaHei"/></font><font><b/><sz val="16"/><color rgb="FFFFFFFF"/><name val="Microsoft YaHei"/></font><font><b/><sz val="10"/><color rgb="FFFFFFFF"/><name val="Microsoft YaHei"/></font></fonts><fills count="5"><fill><patternFill patternType="none"/></fill><fill><patternFill patternType="gray125"/></fill><fill><patternFill patternType="solid"><fgColor rgb="FF10233F"/><bgColor indexed="64"/></patternFill></fill><fill><patternFill patternType="solid"><fgColor rgb="FFFDE9D9"/><bgColor indexed="64"/></patternFill></fill><fill><patternFill patternType="solid"><fgColor rgb="FFF5F8FA"/><bgColor indexed="64"/></patternFill></fill></fills><borders count="2"><border/><border><left style="thin"><color rgb="FFD7E0E9"/></left><right style="thin"><color rgb="FFD7E0E9"/></right><top style="thin"><color rgb="FFD7E0E9"/></top><bottom style="thin"><color rgb="FFD7E0E9"/></bottom></border></borders><cellStyleXfs count="1"><xf/></cellStyleXfs><cellXfs count="5"><xf xfId="0" fontId="0" fillId="0" borderId="0"/><xf xfId="0" fontId="1" fillId="2" borderId="0" applyFont="1" applyFill="1"><alignment vertical="center"/></xf><xf xfId="0" fontId="0" fillId="3" borderId="0" applyFill="1"><alignment wrapText="1" vertical="center"/></xf><xf xfId="0" fontId="2" fillId="2" borderId="1" applyFont="1" applyFill="1" applyBorder="1"><alignment wrapText="1" vertical="center"/></xf><xf xfId="0" fontId="0" fillId="4" borderId="1" applyFill="1" applyBorder="1"><alignment wrapText="1" vertical="top"/></xf></cellXfs><cellStyles count="1"><cellStyle name="Normal" xfId="0" builtinId="0"/></cellStyles></styleSheet>`);
