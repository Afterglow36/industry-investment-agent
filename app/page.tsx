...
export default function Home() {
  ...
  const [form, setForm] = useState({ industry: "低空经济", region: "湖北省", entity: "地方国资产业投资平台", focus: "产业链机会、区域布局、可落地项目与尽调优先级" });
  ...
  async function runResearch(mode: "full" | "update", scope?: Scope) {
    ...
    const data = base ? { industry: base.industry, region: base.region, entity: base.entity } : form;
    const now = new Date().toISOString();
    const task: StoredTask = base ? { ...base, status: "running", progress: 2, stage: `正在更新${scope}`, updatedAt: now } : { id, industry: data.industry, region: data.region, entity: data.entity, status: "running", progress: 2, stage: "正在建立研究任务", createdAt: now, updatedAt: now, fileNames: files.map(f => f.name) };
    setTasks(prev => [task, ...prev.filter(t => t.id !== id)]);
    setSelectedId(id); setShowCreate(false); setUpdateScope(null); setBusy(true);
    const body = new FormData();
    body.set("taskId", id); body.set("industry", data.industry); body.set("region", data.region); body.set("entity", data.entity); body.set("mode", mode); body.set("focus", form.focus);
  ...
  return <main className="app-shell">
    ...
    <section className="workspace">
      {!activeTask ? <div className="empty-state"><span>研</span><h1>开始一项新的产业研究</h1><p>支持任意产业、任意区域和不同投资主体。</p><button className="primary-button" onClick={() => setShowCreate(true)}>创建研究任务</button></div> : <>
        <div className="workspace-head"><div><div className="eyebrow"><Badge tone={activeTask.status === "completed" ? "blue" : activeTask.status === "failed" ? "red" : "orange"}>{activeTask.status === "completed" ? "研究已完成" : activeTask.status === "failed" ? "需要处理" : "Agent 正在运行"}</Badge><span>{activeTask.id}</span></div><h1>{activeTask.industry}<small>产业投资研究</small></h1><p>{activeTask.region} · {activeTask.entity}</p></div><div className="head-actions">{result && <><button className="outline-button" onClick={() => downloadXlsx(result)}>下载数据库 .xlsx</button><button className="primary-button" onClick={() => downloadPptx(result)}>直接生成 PPTX ↘</button></>}</div></div>

        {activeTask.status !== "completed" && <section className="run-panel"><div className="run-status"><div className={`agent-orb ${activeTask.status}`}><span>研</span></div><div><small>当前阶段</small><h2>{activeTask.stage}</h2><p>{activeTask.error || "研究过程会实时回传检索、核验和结构化进度。"}</p></div><strong>{activeTask.progress}%</strong></div><div className="master-progress"><i style={{ width: `${activeTask.progress}%` }} /></div><div className="phase-track">{phases.map((p, i) => <div className={i <= stageIndex ? "done" : ""} key={p}><span>{i < stageIndex ? "✓" : `0${i + 1}`}</span><b>{p}</b></div>)}</div>{activeTask.status === "failed" && <button className="outline-button retry" onClick={() => { setForm({ ...form, industry: activeTask.industry, region: activeTask.region, entity: activeTask.entity }); setShowCreate(true); }}>修改配置后重试</button>}</section>}
        ...
        {result && <>
          ...
          <section className="export-panel"><div><Badge tone="orange">REAL FILE OUTPUT</Badge><h2>不是PPT稿子，是可直接打开编辑的PPTX</h2><p>自动套用“四图五清单”结构，包含封面、管理层摘要、四图、五清单、评分、数据缺口和来源索引。</p></div><button onClick={() => downloadPptx(result)}>生成并下载 PowerPoint <b>↘</b></button><button onClick={() => downloadXlsx(result)}>生成并下载研究数据库 <b>↘</b></button></section>
        </>}
      </>}
    </section>

    {showCreate && <div className="modal-backdrop" onClick={() => setShowCreate(false)}><section className="project-modal" onClick={e => e.stopPropagation()}><button className="modal-close" onClick={() => setShowCreate(false)}>×</button><div className="eyebrow"><Badge tone="orange">NEW RESEARCH</Badge><span>任意产业 · 真实在线研究</span></div><h2>创建产业研究任务</h2><div className="form-grid"><label>目标产业<input value={form.industry} onChange={e => setForm({ ...form, industry: e.target.value })} placeholder="例如：商业航天、合成生物、低空经济" /></label><label>核心区域<input value={form.region} onChange={e => setForm({ ...form, region: e.target.value })} /></label><label className="full">实施主体<input value={form.entity} onChange={e => setForm({ ...form, entity: e.target.value })} /></label><label className="full">研究重点<textarea value={form.focus} onChange={e => setForm({ ...form, focus: e.target.value })} /></label></div><div className="upload-zone" onClick={() => fileInput.current?.click()}><input ref={fileInput} type="file" multiple accept=".pdf,.doc,.docx,.ppt,.pptx,.xls,.xlsx,.csv,.txt,.md" onChange={e => setFiles(Array.from(e.target.files || []))} /><span>＋</span><strong>上传内部材料</strong><p>PDF、Word、PPT、Excel、CSV、TXT；最多8份，合计18MB</p>{files.length > 0 && <div>{files.map(f => <em key={f.name}>{f.name} · {(f.size / 1024 / 1024).toFixed(1)}MB</em>)}</div>}</div><div className="output-checks"><span>将自动生成</span><b>产业数据库 .xlsx</b><b>四图五清单</b><b>评分与数据缺口</b><b>可编辑 PPTX</b></div><button className="primary-button launch" disabled={!form.industry || busy} onClick={() => runResearch("full")}>启动真实研究 Agent →</button><p className="security-note">内部材料仅发送给本次研究请求，不写入公开来源；网站不会在浏览器中保存文件内容。</p></section></div>}
